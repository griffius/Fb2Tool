...
from PyQt5.QtWidgets import QMenu, QApplication, QFileDialog
...
class RenameDialog(Ui_RenameDialog, SmartDialog):
    ...
    def onTextFilenameFormatClick(self):
        ...
        base_templates = {
            ...
            _t('ren', 'Author. Title (tr. Translator)'):
                '{author}. {title}{iif(translator, " (пер. " + translator + ")")}',
            _t('ren', 'Author. (Series Index) Title'):
                '{author}. {iif(series, "("+series)}{iif(seriesindex, " " + seriesindex.zfill(2))}{iif(series, ") ")}'
                '{title}',
            _t('ren', 'Author/Author. Title (tr. Translator)'):
                '{path(author)}{author}. {title}{iif(translator, " (пер. " + translator + ")")}',
            _t('ren', 'Author/Author. (Series Index) Title'):
                '{path(author)}{author}. {iif(series, "("+series)}{iif(seriesindex, " " + seriesindex.zfill(2))}'
                '{iif(series, ") ")}{title}',
            _t('ren', 'Author/Series/Index. Title'):
