            ...
            if self.isRunning:
                ...
                try:
                    ...
                    if not os.path.exists(dst) or self.overwtite_exists:
                        ...
                        raise Exception(_t('ren', 'Destination file already exsist'))
                ...
                self.errors.append({'src': None, 'dest': None, 'error': _t('ren', 'User interrupt')})
                break
        self.finished.emit()
...
class MoveFilesDialog(QDialog, Ui_ProcessDialog):
    def __init__(self, parent, book_info_list, filename_format, author_format, translator_format, delete_src, backup_src, overwrite_exists,
        ...
        self.setWindowTitle(_t('ren', 'Rename {0} files').format(len(book_info_list)))
        if delete_src:
            self.operationName = _t('ren', 'Move')
        else:
            self.operationName = _t('ren', 'Copy')
    ...
    def setCurrentProcess(self, index, count):
        self.progressLabel.setText(_t('ren', '{0} files... {1} of {2}').format(self.operationName, index, count))
